...
emg_event_data <- merge(emg_event_data, demData, by = "Subject")
emg_event_data$Subject <- as.factor(emg_event_data$Subject)
...
lme1b <- lme(log_emg_corr_mean_stimulus ~ instruction*valence, data = emg_event_data, random = ~1|Subject, na.action = na.omit)
lme1c <- lme(log_emg_corr_mean_stimulus ~ instruction*Treatment, data = emg_event_data[emg_event_data$valence == "Negative", ], random = ~1|Subject, na.action = na.omit)
...
lme1d <- lme(log_emg_corr_mean_stimulus ~ instruction*valence + IRI_EC_z*instruction + IRI_EC_z*valence, data = emg_event_data, random = ~1|Subject, na.action = na.omit)
plot(lme1d)
summary(lme1d)

# IRI-PT
emg_event_data$IRI_PT_z <- scale(emg_event_data$IRI_PT)
lme2 <- lme(log_emg_corr_mean_stimulus ~ instruction*valence + IRI_PT_z*instruction + IRI_PT_z*valence, data = emg_event_data, random = ~1|Subject, na.action = na.omit)
plot(lme2)
summary(lme2)

# IRI-PD
emg_event_data$IRI_PD_z <- scale(emg_event_data$IRI_PD)
lme3 <- lme(log_emg_corr_mean_stimulus ~ instruction*valence + IRI_PD_z*instruction + IRI_PD_z*valence, data = emg_event_data, random = ~1|Subject, na.action = na.omit)
plot(lme3)
summary(lme3)

# IRI-F
emg_event_data$IRI_F_z <- scale(emg_event_data$IRI_F)
lme4 <- lme(log_emg_corr_mean_stimulus ~ instruction*valence + IRI_F_z*instruction + IRI_F_z*valence, data = emg_event_data, random = ~1|Subject, na.action = na.omit)
plot(lme4)
summary(lme4)

# STAI-T
emg_event_data$STAI.T_z <- scale(emg_event_data$STAI.T)
lme5 <- lme(log_emg_corr_mean_stimulus ~ instruction*valence + STAI.T_z*instruction + STAI.T_z*valence, data = emg_event_data, random = ~1|Subject, na.action = na.omit)
plot(lme5)
summary(lme5)

# TAS-20
emg_event_data$TAS.20_z <- scale(emg_event_data$TAS.20)
lme6 <- lme(log_emg_corr_mean_stimulus ~ instruction*valence + TAS.20_z*instruction + TAS.20_z*valence, data = emg_event_data, random = ~1|Subject, na.action = na.omit)
plot(lme6)
summary(lme6)
...
lme7 <- lme(log_emg_corr_mean_stimulus ~ instruction*valence + PPI_SCI_z*instruction + PPI_SCI_z*valence, data = emg_event_data, random = ~1|Subject, na.action = na.omit)
plot(lme7)
summary(lme7)
...
lme8 <- lme(log_emg_corr_mean_stimulus ~ instruction*valence + PPI_FD_z*instruction + PPI_FD_z*valence, data = emg_event_data, random = ~1|Subject, na.action = na.omit)
plot(lme8)
summary(lme8)
...
lme9 <- lme(log_emg_corr_mean_stimulus ~ instruction*valence + PPI_C_z*instruction + PPI_C_z*valence, data = emg_event_data, random = ~1|Subject, na.action = na.omit)
plot(lme9)
summary(lme9)
