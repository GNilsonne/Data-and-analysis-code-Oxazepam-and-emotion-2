...
# Analyse data
# Make figures
pdf("Fig_SCR_ER1.pdf", width = 5, height = 5)
plot(MeanTimecourses$down_neg ~ MeanTimecourses$time_s, type = "n", frame.plot = F, main = "Skin conductance, Neutral-valence stimuli", xlab = "Time (s)", ylab = "EMG (ratio)", xaxt = "n", yaxt = "n", ylim = c(0.998, 1.02))
...
pdf("Fig_SCR_ER2.pdf", width = 5, height = 5)
plot(MeanTimecourses$down_neg ~ MeanTimecourses$time_s, type = "n", frame.plot = F, main = "Skin conductance, Negative-valence stimuli", xlab = "Time (s)", ylab = "EMG (ratio)", xaxt = "n", yaxt = "n", ylim = c(0.998, 1.02))
rect(3, 0.000001, 5, 10, col = "gray88", border = NA)
abline(v = c(0, 2, 3), lty = 3)
axis(1)
axis(2, at = c(1, 1.01, 1.02))
...
pdf("Fig_SCR_ER4.pdf", width = 5, height = 5)
plot(c(eff1$fit[1], eff1$fit[2]),
     ...
     ylim = c(-0.01, 0.02),
     col = col1,
     main = "Skin conductance, Neutral-valence stimuli"
)
...
pdf("Fig_SCR_ER5.pdf", width = 5, height = 5)
plot(c(eff1$fit[3], eff1$fit[4]),
     ...
     ylim = c(-0.01, 0.02),
     col = col1,
     main = "Skin conductance, Negative-valence stimuli"
)
...
write.csv(summary(lme1)$tTable, file = "SCR_main.csv")
write.csv(summary(lme1d)$tTable, file = "SCR_IRI_EC.csv")
write.csv(summary(lme2)$tTable, file = "SCR_IRI_PT.csv")
write.csv(summary(lme3)$tTable, file = "SCR_IRI_PD.csv")
write.csv(summary(lme4)$tTable, file = "SCR_IRI_F.csv")
write.csv(summary(lme5)$tTable, file = "SCR_STAI_T.csv")
write.csv(summary(lme6)$tTable, file = "SCR_TAS_20.csv")
write.csv(summary(lme7)$tTable, file = "SCR_PPI_R_SCI.csv")
write.csv(summary(lme8)$tTable, file = "SCR_PPI_R_FD.csv")
write.csv(summary(lme9)$tTable, file = "SCR_PPI_R_C.csv")
